...
export default function NewGamePage() {
  ...
  const [message, setMessage] = useState("v1"); // this is our version_label
  const [code, setCode] = useState(
    "<!doctype html><html><body><canvas id='c'></canvas><script>const c=document.getElementById('c');c.width=600;c.height=400;const x=c.getContext('2d');x.fillRect(10,10,50,50);</script></body></html>"
  );
  ...
  async function save() {
    ...
    try {
      ...
      const { data: revision, error: e2 } = await supabase
        .from("game_revisions")
        .insert({
          game_id: game.id,
          version_label: message || "v1",
          code: codeToSave,
          created_by: uid,
        })
        .select("id")
      ...
      // Optional (your DB trigger likely sets this too, but keeping it is fine)
    ...
    <AuthGate>
      <div className="p-6 max-w-4xl mx-auto space-y-4">
        ...
        <div className="grid gap-3">
          ...
          />
          <textarea
            className="border rounded px-3 py-2 font-mono text-xs h-48"
            value={code}
